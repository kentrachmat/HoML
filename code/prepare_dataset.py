...
def convert_to_torch(D_PATH, xs, ys, labels_df):
    """
    create an image based on the given data xs, ys, 
    and labels_df to the designated path D_PATH
    ...
    """
    for key in xs.keys():
        print('Saving', key, 'dataset as images')
        os.makedirs(D_PATH, exist_ok=True)
        data_dir = os.path.join(D_PATH, key)
        os.makedirs(data_dir, exist_ok=True)
        for i in tqdm(range(len(xs[key]))):
            ...
            imsave(
                "{}/{}_{}.png".format(class_dir, key, i),
                np.uint8(img),
                check_contrast=False,
            )


def prepare_dataset_main():
    """
    ...
    xs_raw = {data_set: np.empty((num[data_set], num_fts)) for data_set in DATA_SETS}

    for data_set in DATA_SETS:
        fast_import(xs_raw[data_set], fpath=DATA_PATH + f"/{DATA_NAME}_{data_set}.data")
